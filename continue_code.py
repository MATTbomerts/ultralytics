...
def process_dir(file_dir):
    ...
    for file in files:
        ...
        output_file_path=os.path.join("results",file.split('.')[0]+'.json')
        continue_process(file_path, output_file_path)
        
    

def continue_process(file_path, output_file_path):
    # 计算两个矩形框的重叠部分，返回重叠比例
    ...
    for patient_id, results in patients.items():
        ...
        for idx, (box, images) in enumerate(merged_boxes, 1):
            ...
            bleeding_point = {f'slices {idx}': [str(int(img[0].split("-")[1])) for img in images],"box":str(box),"logits":max_logits}
