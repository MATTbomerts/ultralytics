...
patient_img_path="/mnt/hdd1/zhulu/blood_stage2/blood_anno/PNG/Patient-0018081436/Series-Ax SWAN new"
patient_imgs=[img for img in os.listdir(patient_img_path) if img.endswith(".png")]
patient_imgs=sort_files(patient_imgs) #根据图像的切片号进行排序，后面才能直接根据index进行上下层读取
patient_dict["Patient-0018081436"]=patient_imgs  #切面没有进行排序
...
def patient_build_nii(patient_dict,model,resolution):
    all_data=[]
    count=0  #表示一共有多少层图像
    ...
    for patient,imgs in patient_dict.items():
        ...
        if Phase_imgs_len==0:
            ...
            continue
            # return
        if (Swi_imgs_len-1)!=Phase_imgs_len:
        ...
        for index,img in enumerate(imgs):  #此处是一个病人存在标注的所有层面，此时每一张图像都要进行预测
            ...
            else: # 功能是从txt标注文件中获得出血点的坐标用于后面的预测结果正确与否的判断
    ...
    model_3D.load_state_dict(torch.load(f"parameters/{resolution}/CMB_3DCNN_norm_lr0001.pth"))
...
resolution="high"
patient_build_nii(patient_dict,model,resolution)
