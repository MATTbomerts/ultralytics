...
def train_model(model, train_loader,test_loader, criterion, optimizer,scheduler, num_epochs,resolution):
    ...
    for epoch in tqdm(range(num_epochs),desc="training"):  #迭代轮数
        ...
        if best_loss - test_loss > min_delta:  
            best_loss = test_loss
            torch.save(model.state_dict(), f"parameters/res_model/phase/CMB_3DCNN_norm_lr0001_adamw_focal90_ep100_best.pth")
        ...
        #     break
    torch.save(model.state_dict(), f"parameters/res_model/phase/CMB_3DCNN_norm_lr0001_adamw_focal90_ep100_last.pth")    
...
def val_model(model, valid_loader):
    ...
    for i in range(len(all_files)):
        ...
        patient_dict[patient_name].append((all_files[i],all_labels[i],all_pred_labels[i],all_pred_boxes[i],all_logits[i]))
    ...
    # #整体所有swi预测病人的结果
    # with open(f"txt/FromSwi_all_samples_predict.txt","w") as f:    
    #     for patient in patient_names:
    #         for i in range(len(patient_dict[patient])):
    #             sample_name=patient_dict[patient][i][0]
    ...
    #             pred_label=patient_dict[patient][i][2]
    #             pred_box=patient_dict[patient][i][3]
    #             logits=patient_dict[patient][i][4]
    #             f.write(f"{sample_name:<30}|{label:<5}|{pred_label:<5}|{pred_box}|{logits:<5}\n")

    for patient in patient_names:
        patient_file = f"txt/patient/{patient}.txt"  # 以病人名称命名文件
...
def test_model(model, test_loader):
    ...
    """ 打印所有错误预测的样本 """
    # print("wrong predict samples: ",all_wrong_predict_files)
    with open("secondStage/phase_pred/test_wrong_predict_samples.txt","w") as f:
        for item in all_wrong_predict_files:
            f.write(item)
            f.write("\n")
    
    """ 打印所有预测结果，真实标签 | 预测结果 """
    with open("secondStage/phase_pred/test_all_samples_predict.txt","w") as f:
        for i in range(len(all_files)):
            ...
            box=all_boxes[i]
            logits=all_logits[i]
            f.write(f"{sample_name:<30}|{label:<5}|{pred_label:<5}|{str(box):<10}|{str(logits):<10}\n")
    #endregion  
...
def CMB_3DCNN_main(resolution):
    ...
    file_path="/home/zhulu/workspace/ultralytics/secondStage/test_all_samples_predict.txt"
    ...
    criterion = FocalLoss(alpha=[0.1,0.9],gamma=3)
    # optimizer = optim.Adam(model.parameters(), lr=0.001)
    optimizer = optim.AdamW(model.parameters(), lr=0.001)  #将优化器改为adamw
    # scheduler = ExponentialLR(optimizer, gamma=0.8)
    scheduler = CosineAnnealingLR(optimizer, T_max=100, eta_min=1e-6)  #迭代器换成余弦

    if Train:
        train_loss,test_loss=train_model(model, train_loader,test_loader, criterion, optimizer, scheduler,num_epochs=100,resolution=resolution)
    ...
    if Test:
        model.load_state_dict(torch.load(f"parameters/res_model/phase/CMB_3DCNN_norm_lr0001_adamw_focal90_ep100_last.pth"))
        test_model(model, test_loader)
    
    if Val:
        # model.load_state_dict(torch.load(f"parameters/res_model/phase/CMB_3DCNN_norm_lr0001_adamw_focal90_ep100_last.pth"))
        model.load_state_dict(torch.load(f"parameters/res_model/phase/CMB_3DCNN_norm_lr0001_adamw_focal90_last.pth"))
