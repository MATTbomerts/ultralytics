...
def save_one_box(xyxy, im, file=Path("im.jpg"), gain=1.02, pad=10, square=False, BGR=False, save=True):
    ...
    b = ops.xyxy2xywh(xyxy.view(-1, 4))  # boxes #将原来的xyxy格式转换为xywh格式
    if square:
...
def plot_images(
    ...
    for i in range(bs):  #bs为batch size
        x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
...
def output_to_target(output, max_det=300):
    ...
    for i, o in enumerate(output):
        box, conf, cls = o[:max_det, :6].cpu().split((4, 1, 1), 1) #split按照“1”列的维度分割为（4，1，1）列三个部分
        j = torch.full((conf.shape[0], 1), i)  #300,1
        targets.append(torch.cat((j, cls, ops.xyxy2xywh(box), conf*100), 1))  #在画图时为了便于显示，乘100
    targets = torch.cat(targets, 0).numpy()
    #返回[batch_id, class_id, x, y, w, h, conf]
