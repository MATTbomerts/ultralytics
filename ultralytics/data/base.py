...
class BaseDataset(Dataset):
    ...
    def __init__(
        ...
        self.labels = self.get_labels()  #这里面的操作也仅是一些缓存机制，并没有根据调整的size改变标注的大小
    ...
    def get_image_and_label(self, index):
        # 是一张图像一张图像处理的，里面的操作并没有改变标注框的大小，仅将图像进行Resize了
        """Get and return label information from the dataset."""
        label = deepcopy(self.labels[index])  # requires deepcopy() https://github.com/ultralytics/ultralytics/pull/1948
        label.pop("shape", None)  # shape is for rect, remove it
        label["img"], label["ori_shape"], label["resized_shape"] = self.load_image(index)  #这个操作并没有改变标注框的大小，仅将图像进行Resizel了
        label["ratio_pad"] = (
