...
class Model(nn.Module):
    ...
    def __init__(
        ...
        elif self.is_triton_model(model):  #并不是else操作
            self.model_name = self.model = model
            return

        # Load or create new YOLO model
        if Path(model).suffix in {".yaml", ".yml"}:
            self._new(model, task=task, verbose=verbose)  #重新根据配置文件创建模型
        else:
            #官方的微调训练方式，执行的是else分支
    ...
    #表示方法是类的静态方法，不需要实例化类就可以调用，的确连self都没有
    #一般的情况下对于类中的方法，如需调用需要先创建类的实例instance=Xclass(args)，再进行调用
    #如果加上静态修饰，就可以直接通过类本身，静态调用，类名.的方式（例：Model.is_triton_model）
    @staticmethod  
    ...
    def _load(self, weights: str, task=None) -> None:
        ...
        if Path(weights).suffix == ".pt":
            self.model, self.ckpt = attempt_load_one_weight(weights)  #这个weights中有对应的task任务描述
            self.task = self.model.args["task"]
            #一开始得到的是原本预训练模型的参数（包括数据yaml，迭代轮数）
    ...
    def train(
        self,
        trainer=None,
        **kwargs,  # 就是使用train时传入的参数 data="hospital.yaml", epochs=200, imgsz=512
    ):
        """
        ...
        #在python3.5的版本中，{**a,**b}会将字典"a"和字典b合并成一个新的字典，如果有重复的键
        #后面的键值对会覆盖前面的键值对，所以右边的字典优先级更高
        args = {**overrides, **custom, **kwargs, "mode": "train"}  # highest priority args on the right
        if args.get("resume"):
            args["resume"] = self.ckpt_path

        #短路求值，首先判断or左侧表达式是否为真，如果为真，直接返回左侧表达式的值，否则返回右侧表达式的值
        #下面得到的就是ultralytics/models/yolo/model.py 中的trainer
        # 在下面这一步中就有了batch size,epoch等设置了
        ...
        self.trainer.train()  #这行代码执行训练过程,DetectionTrainer中没有定义train方法，train方法在DetectionTrainer继承的父类中
    ...
    def _smart_load(self, key: str):  #key: "trainer"
        """Load model/trainer/validator/predictor."""
        try:
            #此处的task_map为ultralytics/models/yolo/model.py 路径下的task_map，在当前代码文件中没有实现task_map
