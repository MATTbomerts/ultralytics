...
def patient_build_nii(patient_dict,model):
        ...
        for index,img in enumerate(imgs):  #此处是一个病人存在标注的所有层面，此时每一张图像都要进行预测
            ...
            for i, result in enumerate(results): #单张图像处理的话，i始终是0，因为只有一张图像
                for j, box in enumerate(result.boxes): # j表示是第几个box
                    ...
                    all_data.append([crop_Data,1,file_name,xyxy])  #在真实测试时，是没有标签的，随便给一个真实标签，让其预测分类
    ...
    swi_model_3D.load_state_dict(torch.load(f"parameters/res_model/swi/CMB_3DCNN_norm_lr0001_80.pth"))
    pha_model_3D.load_state_dict(torch.load(f"parameters/res_model/phase/CMB_3DCNN_norm_lr0001_70.pth"))
    # model_3D.load_state_dict(torch.load(f"parameters/{resolution}/CMB_3DCNN_new4.pth"))
    pha_data=predict_model(swi_model_3D,valid_loader) #在真实测试时是没有正确率的
